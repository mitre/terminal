...
func main() {
   ...
   socket := flag.String("socket", "127.0.0.1:5678", "The ip:port of the socket listening post")
   http := flag.String("http", "http://127.0.0.1:8888", "The FQDN of the HTTP listening post")
   verbose := flag.Bool("v", false, "Enable verbose output")
   flag.Parse()

   output.SetVerbose(*verbose)
   output.VerbosePrint(fmt.Sprintf("[*] %s socket location %s", *contact, *socket))

   coms, _ := sockets.CommunicationChannels[*contact]
   coms.Listen(key, *socket, *http, profile)
}